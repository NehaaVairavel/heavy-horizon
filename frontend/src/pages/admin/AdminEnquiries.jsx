            ...
            <tbody>
              {filteredEnquiries.map((enquiry, idx) => (
                <tr key={enquiry._id || idx}>
                  ...
                  <td>{enquiry._id && enquiry._id.length >= 8 ? new Date(parseInt(enquiry._id.substring(0, 8), 16) * 1000).toLocaleDateString() : (enquiry.created_at ? new Date(enquiry.created_at).toLocaleDateString() : 'Recent')}</td>
                  <td>
